...
int main() {
  ...
  if (!(rec_ae1 == sq_ae)) {
    std::cerr
        << "Test 8.5 failed. (sometimes circle may be equal to ellipse)\n";
    return 1;
  }
  Circle b3(b, 3);
  Ellipse cf5(c, f, 5);
  ...
  cf5 = Ellipse(f, c, 5);
  {
    ...
    if (!equals(cf5.perimeter(), per)) {
      std::cerr << "Test 9.1 failed. (ellipse perimeter). Consider this: "
  ...
  abd = Triangle(d, b, a);
  {
    ...
    if (!ok) {
      std::cerr
          << "Test 10.0 failed. "
             "(https://en.wikipedia.org/wiki/Euler's_theorem_in_geometry)\n";
      return 1;
    }
    ...
    // euler circle center lies in the middle of segment between orthocenter and
    // circumcenter
    ok = equals(distance(orc, eulerCircle.center()),
                distance(circ, eulerCircle.center()));
    if (!ok) {
      std::cerr << "Test 10.1 failed. (nine-point circle center)\n";
      return 1;
    }
    // Радиус окружности девяти точек равен половине радиуса описанной
    // окружности
