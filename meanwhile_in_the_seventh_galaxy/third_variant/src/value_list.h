#ifndef MEANWHILE_IN_THE_SEVENTH_GALAXY_THIRD_VARIANT_VALUE_LIST_H
#define MEANWHILE_IN_THE_SEVENTH_GALAXY_THIRD_VARIANT_VALUE_LIST_H
...
struct PopBack<ValueList<First, Second, Tail...>> {
 ...
 public:
  using type = typename PushFront<
      First, typename PopBack<typename PopFront<List>::type>::type>::type;
};
...
struct Get<i, ValueList<Head, Tail...>> {
 ...
 public:
  ...
  static constexpr Type value =
      Get<i - 1, typename PopFront<List>::type>::value;
};
...
struct Set<i, Value, ValueList<Head, Tail...>> {
 ...
 public:
  ...
  using type = typename PushFront<
      Head,
      typename Set<i - 1, Value, typename PopFront<List>::type>::type>::type;
};
...
struct Construct {
  static_assert(i != 0);
  using type =
      typename PushFront<Value, typename Construct<Value, i - 1>::type>::type;
};
...
#endif  /// MEANWHILE_IN_THE_SEVENTH_GALAXY_THIRD_VARIANT_BOOL_LIST_H.
