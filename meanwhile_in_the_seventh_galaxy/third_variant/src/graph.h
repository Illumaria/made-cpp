...
/**
 * Checks The edge.
 * type = true, payload = index of vertex: checks that all Edges::from are = to
 * the index. type = false, payload = vertices count: checks that all Edges::to
 * are < vertices count
 */
...
struct CheckVertices<index, vertices, TypeList<Vertices...>> {
 private:
  ...
  using _Edges =
      typename CheckEdges<false, vertices, typename Head::Edges>::list;
...
struct Graph {
  using Vertices = typename CheckVertices<0, sizeof...(_Vertices),
                                          TypeList<_Vertices...>>::list;
};
