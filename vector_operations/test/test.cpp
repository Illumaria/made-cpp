...
int main() {
  ...
  REPEAT(100) {
    ...
    double res = vec * vec2;
    double res2 =
        (valarr * std::valarray<double>(vec2.data(), vec2.size())).sum();
  ...
  REPEAT(100) {
    ...
    ASSERT_TRUE_MSG(fabs(cross * vec) < EPS && fabs(cross * vec2) < EPS,
                    "Cross product")
    ...
    ASSERT_TRUE_MSG(
        fabs(cross * cross - vec[2] * vec[2] * vec2[0] * vec2[0]) < EPS,
        "Cross product")
  }
