...
struct MoveTester {
  ...
  MoveTester& operator=(const MoveTester&) {
    action = "CA";
    return *this;
  }
  MoveTester& operator=(MoveTester&&) noexcept {
    action = "MA";
    return *this;
  }
};
...
#define ASSERT_EQUAL_MSG(cont1, cont2, msg) \
  ASSERT_TRUE_MSG(                          \
      std::equal(cont1.begin(), cont1.end(), cont2.begin(), cont2.end()), msg)
...
int main() {
...

  {
...

    {
      ...
      ASSERT_TRUE_MSG(std::find(list_task.begin(), list_task.end(), 1000) !=
                          list_task.end(),
                      "list::merge")
    }
  }
